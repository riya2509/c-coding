...
int main()
{
    ...
    // Inputing elements in matrix A from user
    ...
    for(row=0; row<SIZE; row++)
    {
        for(col=0; col<SIZE; col++)
        {
            // Storing each row of matrix A to each column of matrix B 
            B[row][col] = A[col][row];
        }
    }


    // Checking whether matrix A is equal to its transpose or not
    isSymmetric = 1;
